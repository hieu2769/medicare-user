...
const AddMoney = ({ isOpen, onClose, cancelRef, closeModal, openModal }) => {
  ...
  const paymentData = {
    ...
    user_id: user.id,
    desc: `Giao dịch nạp tiền vào ví cho mã người dùng -  ${user.id}`,
    method: paymentMethod,
    payment_method: "Online",
    transaction_type: "Credited",
    description: "Số tiền đã được cộng vào ví người dùng",
  ...
  const reLogin = async () => {
    ...
    if (login.response === 200) {
      ...
      toast({
        title: login.message || "Đã có lỗi xảy ra",
        status: "error",
        duration: 3000,
        isClosable: true,
        position: "top",
  ...
  const AddMoney = async (txnID) => {
    let data = {
      ...
      transaction_type: "Credited",
      description: "Số tiền được cộng vào ví người dùng",
    };
    try {
      setisPaymentLoading(true);
      ...
      setisPaymentLoading(false);
      showToast(toast, "error", "Đã có lỗi xảy ra!");
    }
  };
  ...
  const handleSubmit = () => {
    if (!paymentMethod) {
      return toast({
        title: `Không có phương thức thanh toán nào đang hoạt động!`,
        status: "error",
        duration: 3000,
        isClosable: true,
        position: "top",
    ...
    if (amount < 100) {
      toast({
        title: `Hãy chắc chắn rằng số tiền là ${currency} 100 hoặc cao hơn`,
        status: "error",
        duration: 3000,
        isClosable: true,
        position: "top",
          ...
          <AlertDialogContent p={0}>
            ...
            <AlertDialogBody>
              <FormControl>
                ...
                <FormHelperText>
                  {minAmount
                    ? `Yêu cầu tối thiểu ${minAmount} ${currency} để đặt hàng này`
                    : `Hãy chắc chắn rằng số tiền là ${currency} 100 hoặc cao hơn`}
                </FormHelperText>
              </FormControl>
