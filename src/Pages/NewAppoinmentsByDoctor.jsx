    ...
    if (nextStep < step) {
      ...
      if (step === 1 && (!Doctordetails || !appoinmentType)) {
        showToast(
          toast,
          "error",
          "Vui lòng chọn bác sĩ và loại cuộc hẹn trước khi tiếp tục."
        );
        return;
      }
      ...
      if (step === 3 && !patientDetails) {
        showToast(
          toast,
          "error",
          "Vui lòng chọn bệnh nhân trước khi tiếp tục."
        );
        return;
      }
      setStep(nextStep);
          ...
          >
            ...
            {filteredIntervals?.length ? (
              ...
              <Text color={"red.400"} fontWeight={700} fontSize={"sm"}>
                Không có khung giờ nào có sẵn cho ngày đã chọn.
              </Text>
            )}
          </Box>
        ) : (
...
const Step3 = ({ setPatientDetails, setStep }) => {
  ...
  const onSubmit = async (data) => {
    ...
    try {
      ...
      showToast(toast, "error", "Đã xảy ra lỗi");
    }
  };
...
const Step4 = ({
    ...
    try {
      ...
      if (res.response === 200) {
        if (res.status === false) {
          ...
          showToast(toast, "success", "Mã giảm giá đã được áp dụng");
  ...
  const addAppointment = async () => {
    ...
    try {
      ...
      setisLoading(false);
      if (res.response === 200) {
        showToast(toast, "success", "Đặt lịch hẹn thành công!");
        setAllNull();
        ...
        <Box mt={5}>
          <RadioGroup value={method} fontWeight={500} size={"md"}>
            <Stack>
              ...
              >
                Thanh toán từ ví (Số dư: {currency}
                {userData?.wallet_amount})
              </Radio>
            </Stack>
          </RadioGroup>
          ...
          onClick={() => {
            if (!getSlotStatus(selectedSlot)) {
              showToast(
                toast,
                "error",
                "Khung giờ đã được đặt! Vui lòng thử khung giờ khác"
              );
              return;
            }
            if (timePassed(selectedSlot)) {
              showToast(
                toast,
                "error",
                "Thời gian đã qua! Vui lòng thử khung giờ khác"
              );
              return;
            }
