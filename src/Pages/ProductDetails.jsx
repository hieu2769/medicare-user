...
function ProductDetails() {
  ...
  const addToCart = async (product) => {
    ...
    try {
      ...
      if (res.response === 200) {
        showToast(toast, "success", "Đã thêm vào giỏ hàng!");
            ...
            <Box w={"100%"} flex={1} mt={{ base: "0", md: 4 }}>
              ...
              <Flex gap={2}>
                ...
                <Text fontSize={"md"} fontWeight={600} color={"green.500"}>
                  {offPercent(data.price, data.mrp)} % Giảm
                </Text>
              </Flex>{" "}
              ...
              <Button
                ...
                onClick={() => {
                  if (!user) {
                    return showToast(toast, "error", "Vui lòng đăng nhập trước");
                  }
                  mutation.mutate(data);
                }}
              >
                {checkCart(data) ? "Đã thêm vào giỏ hàng" : "Thêm vào giỏ hàng"}
              </Button>
            </Box>
          </Flex>
        </Box>
