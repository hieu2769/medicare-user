...
const AppointmentSuccess = () => {
  ...
  const event = {
    title: `Lịch hẹn với Bác sĩ ${data?.doct_f_name} ${data?.doct_l_name}`,
    start: `${data?.date}T${data?.time_slots}`,
    description: `Chuyên khoa: ${data?.dept_title}\nLoại: ${data?.type}`,
    location: "Địa điểm phòng khám của bạn",
  };
    ...
    >
      <VStack spacing={4}>
        <Image
          ...
          alt="Success"
        />
        <Heading size="sm">Mã lịch hẹn: #{data.id}</Heading>
        <Badge
          size="sm"
          p={2}
          fontWeight={800}
        ...
        >
          {data.type === "OPD"
            ? "Vui lòng đến phòng khám và quét mã QR được cung cấp để nhận số thứ tự khám bệnh ngay lập tức"
            : "Nhấp vào tham gia cuộc họp hoặc quét mã QR để tham gia."}
        </Text>
        ...
        <Flex gap={4}>
          <Button
            ...
            size={"sm"}
          >
            <Image src="/google.png" w={5} mr={3} /> Thêm vào Lịch
          </Button>
          <Button
            colorScheme="gray"
            onClick={() => {
            ...
            size={"sm"}
          >
            <Image src="/appleLogo.png" w={5} mr={3} /> Thêm vào Lịch
          </Button>
        </Flex>
      </VStack>
    </Box>
